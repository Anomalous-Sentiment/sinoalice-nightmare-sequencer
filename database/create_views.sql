...
CREATE OR REPLACE VIEW allNightmareDetails AS
    SELECT nm.jp_name, nm.en_name, nm.jp_icon_url, nm.en_icon_url, nm.rarity_id, attr.attribute_id, pure_skills.jp_colo_skill_name, combined_skills.jp_colo_skill_desc, pure_skills.en_colo_skill_name, pure_skills.active_colour, combined_skills.en_colo_skill_desc, rk.jp_rank, rk.en_rank, combined_skills.prep_time, combined_skills.effective_time, nm.global, pure_skills.skill_type, ARRAY_REMOVE(ARRAY_AGG (tagTable.sub_tag), NULL) applied_tags, ARRAY_REMOVE(ARRAY_AGG (DISTINCT major.major_tag), NULL) major_tags, ARRAY_REMOVE(ARRAY_AGG(gen.general_tag), NULL) general_tags
    FROM nightmares nm
    INNER JOIN element_attributes attr USING (attribute_id)
    INNER JOIN colosseum_skills combined_skills USING (jp_colo_skill_name, jp_rank)
    INNER JOIN ranks rk USING (jp_rank)
