...
CREATE OR REPLACE VIEW nightmaresWithTags AS
    SELECT nm.jp_name, nm.en_name, nm.jp_icon_url, nm.en_icon_url, attr.attribute, pure_skills.art_mst_id, jp_skills.jp_colo_skill_desc, pure_skills.en_colo_skill_name,jp_skills.en_colo_skill_desc, rk.jp_rank, rk.en_rank, nm.global, ARRAY_AGG (tagTable.sub_tag) applied_tags
    FROM nightmares nm
    INNER JOIN rarities rare USING (rarity_id)
    INNER JOIN element_attributes attr USING (attribute_id)
    INNER JOIN colosseum_skills jp_skills USING (art_mst_id, jp_rank)
    ...
    GROUP BY nm.jp_name, nm.rarity_id, attr.attribute_id, jp_skills.art_mst_id, jp_skills.jp_rank, pure_skills.art_mst_id, rk.jp_rank, tagRel.art_mst_id;
...
CREATE OR REPLACE VIEW allJpNightmareDetails AS
    SELECT nm.nm_name, nm.icon_url, nm.rarity_id, attr.attribute_id, jp_skills.skill_desc, pure_skills.active_colour, jp_skills.prep_time, jp_skills.effective_time, pure_skills.skill_type, ARRAY_REMOVE(ARRAY_AGG (tagTable.sub_tag), NULL) applied_tags, ARRAY_REMOVE(ARRAY_AGG (DISTINCT major.major_tag), NULL) major_tags, ARRAY_REMOVE(ARRAY_AGG(gen.general_tag), NULL) general_tags
    FROM jp_nightmares nm
    INNER JOIN element_attributes attr USING (attribute_id)
    INNER JOIN jp_colo_skills jp_skills USING (art_mst_id)
    --INNER JOIN ranks rk USING (jp_rank)
    ...
    GROUP BY nm.nm_name, nm.icon_url, nm.rarity_id, attr.attribute_id, jp_skills.art_mst_id, jp_skills.jp_rank, tagRel.art_unique_id, pure_skills.active_colour, pure_skills.skill_type;
